...
function checkOtherProducts() {
  if (!(localStorage.d2Installed)) {
    ...
    chrome.runtime.sendMessage(productIDs.d2, {checkExtension: true, sourceExtension: "collusion"},
      function(response) {
        if (response) {
...
function checkForNotification() {
  $.getJSON('https://s3.amazonaws.com/disconnect-notifications/collusion/collusion.json', function(notificationJSON) {
    try {
      ...
      if (notificationJSON.running && !(notifications[notificationJSON.currentName])) {
        if (notificationJSON.type === 'growl') {
          notifications[notificationJSON.currentName] = true;
